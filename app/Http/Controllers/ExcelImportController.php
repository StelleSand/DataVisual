   ...
   /* //初始化数据表,仅为此项目静态演示使用
    ...
    public function orderInsert($row)
    {
        ...
        else
        {
            ...
            //dump($merchandise_id);
            //检测订单是否已存在，防止重复插入
            if($this->orderExist($merchandise_id, $row['sendprint_date']->toDateTimeString()))
                return ;
        }
        //根据商品id插入order
    ...
    //插入一条Power_record记录
    public function powerRecordInsert($row,$create_user)
    {
        $row = $row->toArray();
        //此处查询指定create_user的channel号等信息
        ...
        foreach($channels as $channel)
        {
            ...
            if(!$this->powerRecordExist($row['date'],$channel->id,$row['ch'.$channel->channel_number])) {
